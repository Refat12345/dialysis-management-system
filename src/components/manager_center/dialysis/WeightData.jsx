...
import React, { useState } from 'react';
  ...
  const weightData = {
    ...
    return (
      <div>
        <div className="cardTwo bg-white w-full flex flex-col justify-start rounded-lg ">
      <div className="cardGrid grid grid-cols-2">
        <h4 className="text-right">الوزن قبل الجلسة</h4>
        <h4 className="text-right font-bold">{weightData.weightBefore}</h4>

        <h4 className="text-right">الوزن بعد الجلسة</h4>
        <h4 className="text-right font-bold">{weightData.weightAfter}</h4>

        <h4 className="text-right">معدل السحب الكلي/سا</h4>
        <h4 className="text-right font-bold">{weightData.totalWithdrawalRate}</h4>

        <h4 className="text-right">وقت بداية الجلسة</h4>
        <h4 className="text-right font-bold">{weightData.sessionStart}</h4>

        <h4 className="text-right">وقت نهاية الجلسة</h4>
        <h4 className="text-right font-bold">{weightData.sessionEnd}</h4>
      </div>
    </div>
      </div>
    )
