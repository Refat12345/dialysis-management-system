...
//     }
//     setHighlightedCells(newHighlightedCells);
//   }, [searchTerm, data ,chairNumbers,daysOfWeek]);
...
//                     ${role === "secretary" && data[chairNumber][day].patientName === "لا يوجد" ? `${patientID !=undefined ? 
...
const TableComponent = ({ shift, appointments, chairNumbers, role, searchTerm, patientID }) => {
  ...
  const data = useMemo(() => {
    ...
    appointments.forEach(appointment => {
      if (appointment.shiftName === shift && chairNumbers.includes(appointment.chairNumber)) {
        preparedData[appointment.chairNumber][appointment.day] = {
          patientName: appointment.patientName || "لا يوجد",
          shiftID: appointment.shiftID,
          appointmentID: appointment.id,
