//Text Porperty
export const headlineLargeStyle = "lg:text-3xl md:text-2xl sm:text-xl text-lg";
export const headlineMediumStyle =
  "lg:text-lg md:text-base sm:text-base text-sm";
export const bodyMeduimStyle = "lg:text-base md:text-base sm:text-base text-sm";
export const bodySmallStyle = "lg:text-sm md:text-sm sm:text-xs text-xs";
//Margin Property
export const loginMarginX = "lg:mx-20 md:mx-14 sm:mx-10 mx-5";

//Pading Property
export const loginPaddingX = "lg:px-9 md:px-7 sm:px-4 px-3";
